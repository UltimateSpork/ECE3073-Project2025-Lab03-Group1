#include <stdio.h>
#include "includes.h"
#define VGA_PRIORITY      5
#define VGA_STACKSIZE  2048
OS_STK vga_stk[VGA_STACKSIZE];


volatile int * ADDR_EXPORT  = (int *) 0x04041030;
volatile int * PIXEL_EXPORT = (int *) 0x04041050;
volatile int * WREN_EXPORT  = (int *) 0x04041020;

void write_pixel(unsigned int addr, unsigned char color) {
    *ADDR_EXPORT = addr;
    *PIXEL_EXPORT = color & 0xF;
    *WREN_EXPORT = 1;
    for(volatile int i = 0; i < 1000; i++); // Tiny delay to let the signal settleu
    *WREN_EXPORT = 0;
}
void draw_color_stripes(){

	for (int y = 0; y < 240; y++){
		for (int x = 0; x < 320; x++){
			unsigned int addr = y * 320 + x;
			unsigned char color = x/20;
			write_pixel(addr, color);
		}
	}
}

void draw_vertical_stripes(){
	for(int y = 0; y < 240; y++){
		unsigned char color = y /15;
		for (int x = 0; x < 320; x++){
			unsigned int addr = y * 320 + x;
			write_pixel(addr, color);
		}
	}
}

void VGATask(void* pdata){
	printf("Start VGA Task");
	while (1){
		draw_color_stripes();
		printf("Should output horizontal stripes");
		OSTimeDlyHMSM(0,0,1,0);
		draw_vertical_stripes();
		printf("Should output vertical stripes");
		OSTimeDlyHMSM(0,0,1,0);

	}
}

int main()
{

	*ADDR_EXPORT = 0;
	*PIXEL_EXPORT = 0;
	*WREN_EXPORT = 0;
	OSTaskCreateExt(VGATask,
	                    NULL,
	                    (void*)&vga_stk[VGA_STACKSIZE - 1],
	                    VGA_PRIORITY,
	                    VGA_PRIORITY,
	                    vga_stk,
	                    VGA_STACKSIZE,
	                    NULL,
	                    0);
	OSStart();


	return 0;
}
